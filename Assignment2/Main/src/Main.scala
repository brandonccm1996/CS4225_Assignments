...
object Main {
  def main(args: Array[String]) {
    ...
    // Remove stopwords
    val stopWordLines = spark.read.text(args(0))
    val stopWordLinesArray = stopWordLines.collect.map(row=>row.getString(0))

    val tweetsDataFrame = spark.read.format("csv").load(args(1)).select("_c5")
    ...
    val kmeans = new KMeans().setK(400).setFeaturesCol("word2vec_result").setPredictionCol("predicted_cluster")
